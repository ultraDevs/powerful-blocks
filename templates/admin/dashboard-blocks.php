<?php
/**
 * Dashboard - Blocks
 *
 * @package PowerfulBlocks
 */

defined( 'ABSPATH' ) || die();

$block_list = self::all_blocks();
$inactive_blocks = self::inactive_blocks();

?>
<div class="wrap">
	<div class="pb-blocks-list">
		<?php
		foreach ( $block_list as $key => $block ) {

			if ( ! in_array( $key, $inactive_blocks ) ) {
				$checked = 'checked="checked"';
			} else {
				$checked = '';
			}

			if ( ! udpb_has_pro() ) {
				if ( true === $block['is_pro'] ) {
					$checked = 'disabled="disabled"';
				}
			}
			?>
		<div class="pb-w__item">
			<span class="pb-w-i__icon">
				<?php echo udpb_get_block_icon( $key ); ?>
			</span>
			<h3 class="pb-w-i__name">
				<?php
					echo esc_html( $block['title'] );
					echo ( true === $block['is_pro'] ) ? ' <b>(Pro)</b>' : '';
				?>
				<a href="<?php echo udpb_get_demo_link( $key ); ?>" target="_blank">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M288 288a64 64 0 0 0 0-128c-1 0-1.88.24-2.85.29a47.5 47.5 0 0 1-60.86 60.86c0 1-.29 1.88-.29 2.85a64 64 0 0 0 64 64zm284.52-46.6C518.29 135.59 410.93 64 288 64S57.68 135.64 3.48 241.41a32.35 32.35 0 0 0 0 29.19C57.71 376.41 165.07 448 288 448s230.32-71.64 284.52-177.41a32.35 32.35 0 0 0 0-29.19zM288 96a128 128 0 1 1-128 128A128.14 128.14 0 0 1 288 96zm0 320c-107.36 0-205.46-61.31-256-160a294.78 294.78 0 0 1 129.78-129.33C140.91 153.69 128 187.17 128 224a160 160 0 0 0 320 0c0-36.83-12.91-70.31-33.78-97.33A294.78 294.78 0 0 1 544 256c-50.53 98.69-148.64 160-256 160z"/></svg>
			</a></h3>
			<div class="pb-w-i__toggle switch_box">
				<input class="pb-toggle" <?php echo esc_html( $checked ); ?> id="pb-block-<?php echo esc_html( $key ); ?>" type="checkbox" name="blocks[]" value="<?php echo esc_html( $key ); ?>">
			</div>
		</div>
			<?php
		}
		?>
	</div>
</div>